...
export function TablaPagos({ datos, cargando, alHacerClicFila }) {
  ...
  if (cargando) {
    return <div className="flex justify-center items-center h-64 font-bold text-[#00A4E4]">Cargando información...</div>;
  }

  if (!Array.isArray(datos)) {
    return (
      <div className="p-6 border-l-4 border-red-500 bg-red-50">
        <h3 className="text-red-700 font-bold">Error de Sistema</h3>
        <p className="text-red-600 text-sm">El servidor no devolvió el formato esperado.</p>
      </div>
    );
  }

  if (datos.length === 0) {
    return <div className="text-center text-gray-500 p-10 mt-6 border border-gray-200 bg-gray-50">No hay registros que coincidan con los criterios seleccionados.</div>;
  }
          ...
          <tbody className="divide-y divide-gray-100">
            {filasActuales.map((fila, index) => (
              ...
              >
                ...
                <td className="px-6 py-3 truncate max-w-[200px]" title={fila.NOMCLIENT}>{fila.NOMCLIENT}</td>
                <td className="px-6 py-3 text-right font-bold text-[#00A4E4]">{formatoDinero(fila.TOTAL)}</td>
                <td className="px-6 py-3 text-gray-500">{fila.TIPORQ || '-'}</td>
      ...
      <div className="flex justify-between items-center p-4 bg-gray-50 border-t border-gray-200">
        ...
        <div className="flex gap-1">
          <button onClick={() => setPaginaActual(paginaActual - 1)} disabled={paginaActual === 1}
            className="px-3 py-1 border border-gray-300 bg-white text-xs font-bold text-gray-600 disabled:opacity-50 hover:bg-gray-100 transition-colors uppercase">
            Ant
          </button>
          <span className="px-3 py-1 text-xs font-bold bg-[#00A4E4] text-white uppercase">
