    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white/90 backdrop-blur-md border border-slate-200/80 p-4 shadow-[0_10px_30px_rgba(15,23,42,0.06)] rounded-xl font-title z-50 font-tech">
          <p className="font-extrabold text-[9px] text-[#4f46e5] uppercase tracking-widest mb-1.5 neon-text-indigo">
            ESTATUS {label}
          </p>
    ...
    <div className="w-full flex flex-col font-title relative">
      <div className="flex justify-between items-center mb-6 z-10">
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 bg-[#4f46e5] rounded-full cyber-pulse-indigo"></div>
          <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
            Distribución de Inversión por Estado Operativo
          </h3>
        </div>
        <span className="text-[9px] font-bold text-[#4f46e5] bg-[#4f46e5]/5 border border-[#4f46e5]/15 px-2.5 py-0.5 rounded-full uppercase font-tech tracking-wide">
          Valores en MXN
        </span>
      </div>
      ...
      <div className="h-80 w-full z-10">
        {!mounted ? (
          <div className="h-full w-full flex flex-col items-center justify-center bg-white/20 border border-slate-200/30 backdrop-blur-sm rounded-2xl relative overflow-hidden">
            <div className="absolute w-24 h-24 bg-[#4f46e5]/4 blur-xl rounded-full animate-pulse"></div>
            <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#4f46e5] mb-4"></div>
          ...
          <ResponsiveContainer width="100%" height="100%" debounce={300}>
            <BarChart data={dataGrafica} margin={{ top: 10, right: 10, left: 10, bottom: 5 }}>
              ...
              <Bar 
                ...
                radius={[8, 8, 0, 0]} 
                barSize={40} 
                activeBar={{ fill: '#4f46e5', stroke: '#ffffff', strokeWidth: 1.5 }} 
